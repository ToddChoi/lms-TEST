...
export default function CategoryManager({ initialCategories }: Props) {
  ...
  async function handleSaveEdit(id: string) {
    ...
    setLoading(false)
    if (res.ok) {
      setCategories((prev) => prev.map((c) => c.id === id ? { ...c, name: editState.name, slug: editState.slug, description: editState.description || null, icon: editState.icon || null, sort_order: editState.sort_order, is_visible: editState.is_visible } : c))
      setEditingId(null)
      router.refresh()
    } else {
